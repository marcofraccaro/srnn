...
class SRNN_midi(Model):
    """
    The :class:'SRNN_midi' class represents the implementation of Stochastic RNN
    """

    def __init__(self, settings):

        # Call constructor of base model
        super(SRNN_midi, self).__init__()
        ...
        # Mask to handle sequences of different lengths (not needed for the MIDI data, only for TIMIT)
        ...
        # to the way we have preprocessed the data we can use the last hidden state of the current batch to
        ...
        self.d_layer = lasagne.layers.GRULayer(
            ...
            resetgate=lasagne.layers.Gate(W_in=init_rnn, W_hid=init_rnn, W_cell=None),
            updategate=lasagne.layers.Gate(W_in=init_rnn, W_hid=init_rnn, W_cell=None),
            hidden_update=lasagne.layers.Gate(W_in=init_rnn, W_hid=init_rnn, W_cell=None,
        ...
        self.z_init_sh = theano.shared(np.zeros((settings.batch_size,
                                                 settings.latent_size_z),
                                                dtype=theano.config.floatX))

        self.input_layer_z_tm1 = lasagne.layers.InputLayer((None, settings.latent_size_z), name="input_layer_z_tm1")
        ...
        # Compute the sigmoid output and reshape
        p_out_reshaped = lasagne.layers.DenseLayer(generative_mlp, num_units=settings.output_dim,
                                                   nonlinearity=lasagne.nonlinearities.sigmoid,
                                                   name="p_out_reshaped")

        self.p_out_layer = lasagne.layers.ReshapeLayer(p_out_reshaped,
                                                       (settings.batch_size, -1, settings.output_dim),
                                                       name="p_out_layer")

        # List of all layers that we need to pass for pickle/model_info (see base model)
        self.output_layer = [self.z_layer, self.mean_prior_layer, self.log_var_prior_layer, self.p_out_layer]
    ...
    def initialize_computation_graph(self, data, settings):
        ...
        ###############################################################################################################
        d, z, mean_q, log_var_q, mean_prior, log_var_prior, p_out = \
        ...
        def elbo_h_gaussian_x_bernoulli(d, z, mean_q, log_var_q, mean_prior, log_var_prior, p_out, x, mask, settings,
                                        temperature_KL=1.0, test=False):

            if not test:
                mask_sum = T.sum(mask, axis=1)
                # We some over output_dim and we take the mean over batch_size and sequence_length
                log_p_x_given_h = log_bernoulli(x=x, p=p_out, eps=settings.tolerance_softmax) * mask.dimshuffle(0, 1,
                                                                                                                'x')
                ...
                log_p_x_given_h = log_bernoulli(x=x, p=p_out, eps=settings.tolerance_softmax) * mask.dimshuffle(0, 1,
                                                                                                                'x')
                log_p_x_given_h_tot = log_p_x_given_h.sum(axis=(1, 2)) / mask_sum
                ...
                kl_divergence_tot = kl_divergence_seq.sum(axis=(1, 2)) / mask_sum

                lower_bound = log_p_x_given_h_tot - temperature_KL * kl_divergence_tot

                return lower_bound

        lower_bound_train = elbo_h_gaussian_x_bernoulli(d, z, mean_q, log_var_q, mean_prior, log_var_prior,
                                                        p_out, self.x_sym, self.sym_mask, settings,
        ...
        ###############################################################################################################
        d_eval, z_eval, mean_q_eval, log_var_q_eval, mean_prior_eval, log_var_prior_eval, p_out_eval = \
            lasagne.layers.get_output([self.d_layer, self.z_layer, self.mean_q_layer, self.log_var_q_layer,
                                       self.mean_prior_layer, self.log_var_prior_layer,
                                       self.p_out_layer],
                                      inputs={self.input_layer_u: self.u_sym,
        ...
        lower_bound_eval = elbo_h_gaussian_x_bernoulli(d_eval, z_eval, mean_q_eval, log_var_q_eval, mean_prior_eval,
                                                       log_var_prior_eval, p_out_eval, self.x_sym, self.sym_mask,
                                                       settings, test=True)
